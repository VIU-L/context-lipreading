...
class HubertFeatureReader(object):
    ...
    def load_feature(self, mix_name, ref_len=None):
        video_fn, _ = mix_name  # ignore audio path completely
        video_feats = self.load_image(video_fn)

        # audio_feats shape: (num_frames, feature_dim)
        # num_frames = len(video_feats), feature_dim = self.stack_order_audio * 26
        num_frames = len(video_feats)
        feature_dim = self.stack_order_audio * 26  # same as FBANK stacking
        audio_feats = np.zeros((num_frames, feature_dim), dtype=np.float32)
    ...
    def get_feats(self, path, ref_len=None):
        ...
        with torch.no_grad():
            ...
            # print(video_feats.shape, audio_feats.shape) # torch.Size([1, 1, 123, 88, 88]) torch.Size([1, 104, 123])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("--tsv_dir",default="/mnt/sdb/yuran/av_hubert/datasets/multivsr/lrs3_format/data")
    parser.add_argument("--split",default="train")
    parser.add_argument("--ckpt_path",default="/mnt/sdb/yuran/av_hubert/ckpts/large_vox_iter5.pt")
    parser.add_argument("--layer", type=int,default=12)
    parser.add_argument("--nshard", type=int,default=1)
    parser.add_argument("--rank", type=int,default=0)
    parser.add_argument("--feat_dir",default="/mnt/sdb/yuran/av_hubert/datasets/multivsr/features/from_official_pretrained/Layer_12")
    parser.add_argument("--max_chunk", type=int, default=1600000)
    parser.add_argument("--user_dir", type=str, default="/mnt/sdb/yuran/av_hubert_llm/VSP-LLM/src")
    parser.add_argument('dummy', type=str, default=None, help='A dummy argument for future use.')
